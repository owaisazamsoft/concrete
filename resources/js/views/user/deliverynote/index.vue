    ...
    <v-col cols="12">
      <v-card>
        <v-card-title class="d-flex align-center justify-space-between">
          <div>
               <div class="text-h6">Delivery Notes</div>
               <div class="text-caption text-grey">View All Delivery Notes Details</div>
          </div>
          <ToolbarOption>
              <v-list-item title="Create" :to="'/user/deliverynote/create'" />
          </ToolbarOption>
        </v-card-title>
        <v-card-text>
          ...
          <v-data-table-server 
            ...
            <template #item.actions="{ item }">
                <ToolbarOption>
                    <v-list-item title="Edit" :to="'/user/deliverynote/edit/'+item.id" />
  ...
  methods: {
    ...
    async loadItems() {
        ...
        try {
            ...
            const res = await generaApi.get('/api/deliveryNotes',this.filter);
            this.data = res.data;
    ...
    async DeleteRecord(item) {
        if (!confirm("Are you sure you want to delete selected items?")) return ;
            this.loading = true;
            try {
                await generaApi.delete('/api/deliveryNotes/'+item.id,{});
